...
It would be nice to have a function, that returns an array (or a list in Java/Kotlin and C#) of 
...
function getPINs(observed) {
  var array = [
    ...
    TwoDArray = observed.split("").map((e) => array[e]),
    PnC = [],
    maxComb = 1;
  TwoDArray.forEach((e, i, arr) => maxComb *= arr[i].length);
  while (PnC.length !== maxComb) {
    var randComb = "";
    TwoDArray.forEach((e, i, arr) => randComb += arr[i][Math.floor(Math.random() * arr[i].length)]);
    PnC.push(randComb);
    PnC = [...new Set(PnC)];
  }
  return PnC;
